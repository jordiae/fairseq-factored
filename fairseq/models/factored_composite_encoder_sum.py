...
class FactoredCompositeEncoderSum(FairseqEncoder):
    """
    A wrapper around a dictionary of :class:`FairseqEncoder` objects.

    We run forward on each encoder and return their addition.
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        for index, key in enumerate(self.encoders):
            ...
            if sum_encoder is None:
                sum_encoder = encoder_out
            else:
                s = torch.add(sum_encoder['encoder_out'], encoder_out['encoder_out'])
                sum_encoder['encoder_out'] = s
        return sum_encoder
